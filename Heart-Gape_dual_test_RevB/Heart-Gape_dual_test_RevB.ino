...
enum sensorMode {
  ...
  GAPE    // equivalent to 1
};
char sensorMode = 0; // Used to set gape or heart mode

enum serialHeartMode {
  FASTPLOT,     // equivalent to 0
  OLED          // equivalent to 1                  
};
bool serialHeartMode = 0;
...
void setup() {
  ...
  sensorMode = digitalRead(SENSOR_SELECT); // check if we're reading heart or gape
  serialHeartMode = digitalRead(DIP1); // check which position the OLED/FASTPLOT switch is in
  
  IRledBrightness = SampleDIPswitchIR(); // check the desired IR brightness
  ...
  if (sensorMode == GAPE) {
    ...
    if ( serialHeartMode == OLED ){
      ...
      oled.print(" mA  mA");
      digitalWrite(GRNLED, LOW);
    } else if ( serialHeartMode == FASTPLOT ){
      // Serial plotter output mode
      loopDelayMS = 50; // Speed up the main loop
      oled.println("Heart");
      oled.println("Serial");
      oled.println("Plotter");
      oled.print("IR = "); oled.print(IRledBrightness);
    }
...
void loop() {
  ...
  if (millis() - oldMillis > loopDelayMS) {
    ...
    if (sensorMode == HEART) { // Heart mode
      //------------- Heart mode --------------------------
...
//          movingAverageSleepCurr = movingAverageSleepCurr / (actualShuntResistance / 0.1);
    ...
    } else if (sensorMode == GAPE) { // Gape mode
      //------------- Gape mode ------------------------
      loopCounter++; 
